...
################################################################################
# STARTEKK - AUTOMATED PAGE UPDATE & DEPLOYMENT SCRIPT (WINDOWS COMPATIBLE)
# 
...
# Create temp directory in current folder (Windows compatible)
mkdir -p .temp
...
# Create Python script to update header/footer (use current directory)
...
cat > .temp/fix_hero_finance.py << 'PYTHON_EOF'
import re
...
python .temp/fix_hero_finance.py
...
cat > .temp/fix_hero_automation.py << 'PYTHON_EOF'
import re
...
python .temp/fix_hero_automation.py
